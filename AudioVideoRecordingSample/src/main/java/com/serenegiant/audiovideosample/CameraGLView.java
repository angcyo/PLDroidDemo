...
/*		// the frequency of refreshing of camera preview is at most 15 fps
    ...
    private void calcViewport() {
        float scaling = mVideoWidth / (float) mVideoHeight;

        int w, h;

        h = mVideoHeight;
        w = (int) (mVideoHeight * scaling);

        mRenderViewport.width = w;
        mRenderViewport.height = h;
        mRenderViewport.x = (mVideoWidth - mRenderViewport.width) / 2;
        mRenderViewport.y = (mVideoHeight - mRenderViewport.height) / 2;
        Log.e("angcyo", String.format("View port: %d, %d, %d, %d", mRenderViewport.x, mRenderViewport.y, mRenderViewport.width, mRenderViewport.height));
    }
        ...
        queueEvent(new Runnable() {
            ...
            public void run() {
                ...
                if (mRenderer.init(w, h, w, h)) {
                    ...
                    Log.e("angcyo", "Frame Recorder init Ok! -- ");
                } else {
                    Log.e("angcyo", "Frame Recorder init failed! -- ");
                }
                //--end
                mRenderer.updateViewport();
            }
...
//********************************************************************************
     ...
     */
    private static final class CameraSurfaceRenderer extends CGEFrameRenderer implements GLSurfaceView.Renderer, SurfaceTexture.OnFrameAvailableListener {    // API >= 11
...
//			GLDrawer2D.deleteTex(hTex);//robi
            release();
        }
        ...
        public void onDrawFrame(final GL10 unused) {
            ...
            if (requesrUpdateTex) {
                ...
                //robi
                update(hTex, mStMatrix);
                //--end
            }
            // draw to preview screen
//			mDrawer.draw(hTex, mStMatrix);//robi
...
//            render(0, 0, 1280, 720);
            ...
            render(parent.mRenderViewport.x, parent.mRenderViewport.y,
                    parent.mRenderViewport.width, parent.mRenderViewport.height);
